...
int main()
{
    ...
    const char newImgName[]= "girlface_sb_ver.bmp";

    struct Mask sbMask;
    signed char *tmp;
    int i;

    sbMask.Cols=sbMask.Rows =3;
    sbMask.Data = (unsigned char *)malloc(9);
   ...
   /* Vertical Sobel
      ...
      -1    0   1
   */
   short  int sobel_v[] ={-1,0,1,-2,0,2,-1,0,1};
     /*  Horizontal Sobel
       -1   -2    -1
        0    0     0
        1    2      1
     */
  int sobel_h[]  ={-1,-2,-1,0,0,0,1,2,1};

  tmp = (signed char *)sbMask.Data;
  for(int i =0;i<9;i++)
  {
      *tmp = sobel_v[i];
      ++tmp;
       ...
       Convolve(imgHeight,imgWidth,&sbMask,imgBuffer,imgBuffer2);
