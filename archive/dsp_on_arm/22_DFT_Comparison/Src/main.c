...
extern float32_t  hpf_400hz_impulse_response[HPF_100HZ_IMP_RESP_LEN];
...
float REX[_10HZ_100HZ_500HZ_SIG_LEN/2];
float IMX[_10HZ_100HZ_500HZ_SIG_LEN/2];
...
int main()
{
	...
	/*Initialize the uart*/
	 uart2_tx_init();
	  ...
	  calc_sig_dft((float32_t*)_10Hz_100Hz_500Hz_signal,
	 				  (float32_t *)REX,
	 				  (float32_t *)IMX,
...
void serial_plot_rex(void)
{
	...
	for( int j = 0; j < (_10HZ_100HZ_500HZ_SIG_LEN/2); j++)
	{
		printf("%f\r\n",REX[j]);
		pseudo_dly(9000);
...
void get_dft_output_mag(void)
{
	for( int k = 0; k < (_10HZ_100HZ_500HZ_SIG_LEN/2); k++)
	{
		REX[k] = fabs(REX[k]);
	}
}
