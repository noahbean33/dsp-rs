...
// #define DESIRED_AUDIO_INPUT_INTERFACE       "TUSBAudio ASIO Driver" //
// "Thesycon USB BAudio ASIO Driver" #define DESIRED_AUDIO_OUTPUT_INTERFACE
// "TUSBAudio ASIO Driver"              // "Thesycon USB BAudio ASIO Driver"
// #define DESIRED_AUDIO_INPUT_INTERFACE       "ASIO4ALL v2" // UCA202 - ASIO
// Driver #define DESIRED_AUDIO_OUTPUT_INTERFACE      "ASIO4ALL v2" // UCA202 -
// ASIO Driver
...
// For compilers that support precompilation, includes "wx/wx.h".#include
// "wx/wxprec.h"
...
#define PA_SAMPLE_LENGTH \
  2048L                                                     // Length of the sample sequence - longer than I/O length to support
                                                            // delays
    ...
    : wxPanel(parent, id)
{
  ...
  HSizer_AnalysisType->Add(new wxStaticText(this, -1, "Analysis Type : "), 0, wxALIGN_TOP | wxALL,
                           0);    // A combo box for chosing the analysis type
  static const wxString FilterChoices[] = {
      "Time - Triggered",  "Time - Continuous",     "Frequency - Triggered", "Frequency - Continuous",   "Chirp - Time",
      "Chirp - Frequency", "Chirp - X correlation", "Chirp (+Ref) - Time",   "Chirp (+Ref) - Frequency", "Chirp (+Ref) - X correlation",
  };
  ProcessingTypeComboBox = new wxComboBox(this, ID_EVENT_TYPE_CHANGED, wxEmptyString, wxDefaultPosition, wxDefaultSize, WXSIZEOF(FilterChoices),
  ...
  DataSetSizeSpinCtrl = new wxSpinCtrl(this, ID_EVENT_TD_DATASET_SIZE_SPIN_CHANGED, _T (""), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 10,
  ...
  TimeTriggerLevelSpinCtrl =
      new wxSpinCtrl(this, ID_EVENT_TRIG_LEVEL_SPIN_CHANGED, _T (""), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 32, 0);
  ...
  FFTOnePoleSpinCtrl = new wxSpinCtrl(this, ID_EVENT_ONE_POLE_SPIN_CHANGED, _T (""), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 99, 0);
  ...
  FFTPeakHoldSpinCtrl = new wxSpinCtrl(this, ID_EVENT_PEAK_HOLD_SPIN_CHANGED, _T (""), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 99, 0);
  ...
  SIF_Window(pFDPWindow, SIGLIB_HANNING, SIGLIB_ZERO,
  ...
  if ((IOData.pCh0InputSamples == NULL) || (IOData.pCh0OutputSamples == NULL) || (IOData.pCh1InputSamples == NULL) ||
      (IOData.pCh1OutputSamples == NULL)) {
...
void SAProcWindow::OnDataSetSizeSpinControlChange(wxSpinEvent& WXUNUSED(event))
{
  ...
  //  wxMessageBox ("Got here", "SAProcWindow::OnDataSetSizeSpinControlChange",
  ...
  //  {
  //      GlobalGraphDisplayWindow->SetDataSetSize
  //      ((int)DataSetSizeSpinCtrl->GetValue());
  //  }
}
...
void SAProcWindow::OnTriggerLevelSpinControlChange(wxSpinEvent& WXUNUSED(event))
{
  ...
  //  wxMessageBox ("Got here", "SAProcWindow::OnTriggerLevelSpinControlChange",
  //  wxOK | wxICON_EXCLAMATION, NULL);
...
void SAProcWindow::OnOnePoleFilterSpinControlChange(wxSpinEvent& WXUNUSED(event))
{
  ...
  //  "SAProcWindow::OnOnePoleFilterSpinControlChange", wxOK |
...
void SAProcWindow::OnPeakHoldSpinControlChange(wxSpinEvent& WXUNUSED(event))
{
  ...
  //  wxMessageBox ("Got here", "SAProcWindow::OnPeakHoldSpinControlChange",
  //  wxOK | wxICON_EXCLAMATION, NULL);
...
{
  if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME_CONT)) {
    DataSetSizeSpinCtrl->Enable(TRUE);
  ...
  else if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_FREQ) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_FREQ_CONT)) {
    ...
    FFTPeakHoldSpinCtrl->Enable(TRUE);
            ...
            // ID_PROC_CHIRP_REF_TIME, ID_PROC_CHIRP_REF_FREQ,
...
void SAProcWindow::OnProcessData(wxCommandEvent& WXUNUSED(event))
{
  ...
  //  wxMessageBox ("Got Here", "SAProcWindow::OnProcessData", wxOK |
  //  wxICON_EXCLAMATION, NULL);
  ...
  if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME_CONT)) {
    ...
    if (TimeTriggerEnableCheckBox->GetValue() == TRUE) {
      ...
      if (TriggerLocation > (SAMPLE_LENGTH - MAXIMUM_GRAPH_WIDTH)) {    // If trigger location too
                                                                        // late then set to beginning
    ...
    SDA_Copy(pFDPRealData + TriggerLocation, pData,
             MAXIMUM_GRAPH_WIDTH);                 // Copy from start to display array
...
#else
    {    // When SigLib is not being used, copy the input data so that it can be
      // displayed
      int i;
    ...
    SDA_Window(pFDPRealData, pFDPRealData, pFDPWindow,
    ...
    SDA_LogMagnitude(pFDPRealData, pFDPImagData, pData,
                     MAXIMUM_GRAPH_WIDTH);    // Calc real power fm complex
    ...
    SDA_Offset(pData, -138.0, pData,
               MAXIMUM_GRAPH_WIDTH);    // Offset graph results
#else
    wxMessageBox("This functionality is only available when SigLib is installed\nFor "
                 "more details, please see http://www.numerix-dsp.com",
    ...
    SDA_SignalGenerate(pChirpData, SIGLIB_CHIRP_LIN, 32767.0, SIGLIB_FILL, 0.001, 0.0, (0.498 / ((double)CHIRP_LENGTH)), 0.5, &ChirpPhase,
                       &ChirpValue, CHIRP_LENGTH);
    SampleAndGenerateAudio(pFDPRealData, pChirpData,
                           SAMPLE_LENGTH);    // Sample and generate analog data
    if (DCRemoveCheckBox->GetValue() == TRUE) {
      SDA_SubtractMean(pFDPRealData, pFDPRealData, SIGLIB_ONE / (SLData_t)SAMPLE_LENGTH, SAMPLE_LENGTH);
    }
    SDA_Copy(pFDPRealData, pData,
             MAXIMUM_GRAPH_WIDTH);    // Copy input data to display buffer
    ...
    SDA_Divide(pFDPRealData, 32767.0, pFDPRealData,
               SAMPLE_LENGTH);                                                                                 // Re-scale results
    SDA_Rfft(pFDPRealData, pFDPImagData, pFDPFFTCoeffs, SIGLIB_NULL_FIX_DATA_PTR, FFT_SIZE, LOG2_FFT_SIZE);    // Perform FFT
    SDA_LogMagnitude(pFDPRealData, pFDPImagData, pData,
                     MAXIMUM_GRAPH_WIDTH);    // Calc real power fm complex
    SDA_Offset(pData, -28.0, pData,
               MAXIMUM_GRAPH_WIDTH);    // Offset graph results
    SUF_MemoryFree(pChirpData);         // Free memory
#else
    wxMessageBox("This functionality is only available when SigLib is installed\nFor "
    ...
    if ((pFDPRealData == NULL) || (pFDPImagData == NULL) || (pFDPChirpRealData == NULL) || (pFDPChirpImagData == NULL) || (pFDPFFTCoeffs == NULL)) {
      wxMessageBox("Memory allocation failure", "SAProcWindow::OnGetProcessDetails", wxOK | wxICON_EXCLAMATION, NULL);
      return;
    }
    ...
    SIF_Fft(pFDPFFTCoeffs, SIGLIB_NULL_FIX_DATA_PTR,
            XCORR_FFT_SIZE);    // Initialise FFT
    // Generate chirp of SAMPLE_LENGTH from 0.001 to 0.499 - normalised to 1Hz
    double ChirpPhase = SIGLIB_ZERO;
    double ChirpValue = SIGLIB_ZERO;
    SDA_SignalGenerate(pFDPChirpRealData, SIGLIB_CHIRP_LIN, 32767.0, SIGLIB_FILL, 0.001, 0.0, (0.498 / ((double)CHIRP_LENGTH)), 0.5, &ChirpPhase,
                       &ChirpValue, CHIRP_LENGTH);
    SampleAndGenerateAudio(pFDPRealData, pFDPChirpRealData,
                           SAMPLE_LENGTH);    // Sample and generate analog data
    if (DCRemoveCheckBox->GetValue() == TRUE) {
      SDA_SubtractMean(pFDPRealData, pFDPRealData, SIGLIB_ONE / (SLData_t)SAMPLE_LENGTH, SAMPLE_LENGTH);
    }
    SDA_Divide(pFDPRealData, 32767.0, pFDPRealData,
               SAMPLE_LENGTH);    // Re-scale results
    SDA_Divide(pFDPChirpRealData, 32767.0, pFDPChirpRealData,
               SAMPLE_LENGTH);    // Re-scale results
    SDA_Rfft(pFDPChirpRealData, pFDPChirpImagData, pFDPFFTCoeffs, SIGLIB_NULL_FIX_DATA_PTR, XCORR_FFT_SIZE,
             LOG2_XCORR_FFT_SIZE);    // Perform FFT
    SDA_Negate(pFDPChirpImagData, pFDPChirpImagData,
               XCORR_FFT_SIZE);    // Conjugate the data
    SDA_Rfft(pFDPRealData, pFDPImagData, pFDPFFTCoeffs, SIGLIB_NULL_FIX_DATA_PTR, XCORR_FFT_SIZE,
             LOG2_XCORR_FFT_SIZE);    // Perform FFT
    SDA_ComplexMultiply2(pFDPRealData, pFDPImagData, pFDPChirpRealData, pFDPChirpImagData, pFDPRealData, pFDPImagData, XCORR_FFT_SIZE);
    SDA_Cifft(pFDPRealData, pFDPImagData, pFDPFFTCoeffs, SIGLIB_NULL_FIX_DATA_PTR, XCORR_FFT_SIZE,
              LOG2_XCORR_FFT_SIZE);    // Perform inverse FFT
    SDA_Magnitude(pFDPRealData, pFDPImagData, pData,
                  MAXIMUM_GRAPH_WIDTH);    // Calc real power fm complex
    ...
    SDA_SignalGenerate(pChirpData, SIGLIB_CHIRP_LIN, 32767.0, SIGLIB_FILL, 0.001, 0.0, (0.498 / ((double)CHIRP_LENGTH)), 0.5, &ChirpPhase,
                       &ChirpValue, CHIRP_LENGTH);
    SampleAndGenerateAudioWithReference(pFDPRealData, pChirpData, SAMPLE_LENGTH,
                                        CalTholdSpinCtrl->GetValue());    // Sample and generate analog data
    if (DCRemoveCheckBox->GetValue() == TRUE) {
      SDA_SubtractMean(pFDPRealData, pFDPRealData, SIGLIB_ONE / (SLData_t)SAMPLE_LENGTH, SAMPLE_LENGTH);
    }
    SDA_Copy(pFDPRealData, pData,
    ...
    SDA_SignalGenerate(pFDPChirpRealData, SIGLIB_CHIRP_LIN, 32767.0, SIGLIB_FILL, 0.001, 0.0, (0.498 / ((double)CHIRP_LENGTH)), 0.5, &ChirpPhase,
                       &ChirpValue, CHIRP_LENGTH);
    SampleAndGenerateAudioWithReference(pFDPRealData, pFDPChirpRealData, SAMPLE_LENGTH,
                                        CalTholdSpinCtrl->GetValue());    // Sample and generate analog data
    if (DCRemoveCheckBox->GetValue() == TRUE) {
      SDA_SubtractMean(pFDPRealData, pFDPRealData, SIGLIB_ONE / (SLData_t)SAMPLE_LENGTH, SAMPLE_LENGTH);
    }
  ...
  // configure the graph
  if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME_CONT) ||
      (ProcessingTypeComboBox->GetSelection() == ID_PROC_CHIRP_TIME) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_CHIRP_REF_TIME)) {
  ...
  else if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_CHIRP_XCORR) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_CHIRP_REF_XCORR)) {
...
void SAProcWindow::OnCalibrateIO(wxCommandEvent& WXUNUSED(event))
{
  ...
  SDA_SignalGenerate(pChirpData, SIGLIB_CHIRP_LIN, 32767.0, SIGLIB_FILL, 0.001, 0.0, (0.498 / ((double)CHIRP_LENGTH)), 0.5, &ChirpPhase,
                     &ChirpValue, CHIRP_LENGTH);
  CalibrateAudio(pFDPRealData, pChirpData, SAMPLE_LENGTH,
                 CalTholdSpinCtrl->GetValue());    // Sample and generate analog data
  SUF_MemoryFree(pChirpData);                      // Free memory
#else
  wxMessageBox("This functionality is only available when SigLib is installed\nFor more "
               "details, please see http://www.numerix-dsp.com",
               "System Analyzer", wxOK | wxICON_EXCLAMATION, NULL);
#endif
...
void SAProcWindow::OnIdle(wxIdleEvent& WXUNUSED(event))
{
  ...
  //  wxMessageBox ("Got Here", "SAProcWindow::OnIdle", wxOK |
  //  wxICON_EXCLAMATION, NULL);
  if ((ProcessingTypeComboBox->GetSelection() == ID_PROC_TIME_CONT) || (ProcessingTypeComboBox->GetSelection() == ID_PROC_FREQ_CONT)) {
...
static int PlayRecMonoCallback(const void* inputBuffer, void* outputBuffer,
                               unsigned long framesPerBuffer,    // Number of frames (samples) to write to or
                                                                 // read from driver
                               const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags statusFlags, void* userData)
{
  ...
  if (OutputSamplesLeft < ((long)framesPerBuffer)) {    // Number of samples to write out is less than
                                                        // driver buffer size
...
long SampleAudio(double* pInputData, long DataLength)
{
...
#if DEBUG_IO
    SUF_Debugfprintf("SampleAudio - Device %d = %s, inputs = %d, outputs = %d, "
                     "Host API = %s\n",
                     (int)i, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,
                     Pa_GetHostApiInfo(deviceInfo->hostApi)->name);
#endif
    if ((strstr(deviceInfo->name, DESIRED_AUDIO_INPUT_INTERFACE) != 0) &&
        (strcmp(Pa_GetHostApiInfo(deviceInfo->hostApi)->name, DESIRED_AUDIO_API) == 0) && (deviceInfo->maxInputChannels != 0)) {
      InputDeviceNo = i;
    }
    if ((strstr(deviceInfo->name, DESIRED_AUDIO_OUTPUT_INTERFACE) != 0) &&
        (strcmp(Pa_GetHostApiInfo(deviceInfo->hostApi)->name, DESIRED_AUDIO_API) == 0) && (deviceInfo->maxOutputChannels != 0)) {
      OutputDeviceNo = i;
    }
  }
  ...
  for (i = 0; i < SAMPLE_LENGTH; i++) {                              // Save input data for calling function
    *(pInputData + i) = (double)IOData.pCh0InputSamples[i + 100];    // Offset of 100 is used to allow
                                                                     // for latency through ADC
  }
#if DEBUG_IO
...
long SampleAndGenerateAudio(double* pInputData, double* pOutputData, long DataLength)
{
  ...
  for (i = 0; i < numDevices; i++) {
    deviceInfo = Pa_GetDeviceInfo(i);
#if DEBUG_IO
    SUF_Debugfprintf("SampleAndGenerateAudio - Device %d = %s, inputs = %d, "
                     "outputs = %d, Host API = %s\n",
                     (int)i, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,
                     Pa_GetHostApiInfo(deviceInfo->hostApi)->name);
#endif
    if ((strstr(deviceInfo->name, DESIRED_AUDIO_INPUT_INTERFACE) != 0) &&
...
long SampleAndGenerateAudioWithReference(double* pInputData, double* pOutputData, long DataLength, long CalThreshold)
{
  ...
  for (i = 0; i < numDevices; i++) {
    deviceInfo = Pa_GetDeviceInfo(i);
#if DEBUG_IO
    SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Device %d = %s, "
                     "inputs = %d, outputs = %d, Host API = %s\n",
                     (int)i, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,
                     Pa_GetHostApiInfo(deviceInfo->hostApi)->name);
#endif
    if ((strstr(deviceInfo->name, DESIRED_AUDIO_INPUT_INTERFACE) != 0) &&
...
#if DEBUG_IO
  SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Desired audio input "
                   "interface = %s\n",
                   DESIRED_AUDIO_INPUT_INTERFACE);
  SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Desired audio output "
                   "interface = %s\n",
                   DESIRED_AUDIO_OUTPUT_INTERFACE);
  ...
  outputParameters.hostApiSpecificStreamInfo = NULL;
#if DEBUG_IO
  SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Printing device "
                   "details to status line\n");
#endif
  {    // Print the selected device to the status line
    deviceInfo = Pa_GetDeviceInfo(inputParameters.device);
    wxString devString = "I/P = ";
  ...
  err = Pa_OpenStream(&stream, &inputParameters, &outputParameters, SAMPLE_RATE, FRAMES_PER_BUFFER,
                      paClipOff,    // We won't output out of range samples so
                                    // don't bother clipping them
                      PlayRecStereoCallback, &IOData);
  if (err != paNoError) {

#if DEBUG_IO
    SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Pa_OpenStream() "
                     "error_terminate : %d\n",
                     err);
...
#if DEBUG_IO
    SUF_Debugfprintf("SampleAndGenerateAudioWithReference - Pa_StartStream() "
                     "error_terminate : %d\n",
                     err);
...
long CalibrateAudio(double* pInputData, double* pOutputData, long DataLength, long CalThreshold)
{
  ...
  for (i = 0; i < numDevices; i++) {
    deviceInfo = Pa_GetDeviceInfo(i);
#if DEBUG_IO
    SUF_Debugfprintf("CalibrateAudio - Device %d = %s, inputs = %d, outputs = "
                     "%d, Host API = %s\n",
                     (int)i, deviceInfo->name, (int)deviceInfo->maxInputChannels, (int)deviceInfo->maxOutputChannels,
                     Pa_GetHostApiInfo(deviceInfo->hostApi)->name);
#endif
