...
int main()
{
    ...
    const char newImgName[]= "girlface_rbson_west.bmp";

    struct Mask rbsonMask;
    signed char *tmp;
    int i;

    rbsonMask.Cols=rbsonMask.Rows =3;
    rbsonMask.Data = (unsigned char *)malloc(9);
    ...
    int rb_west[]  = {1,2,1,0,0,0,-1,-2,-1};
       ...
       tmp = (signed char *)rbsonMask.Data;
       for(i =0;i<9;++i){
        *tmp =rb_west[i];
        ++tmp;
       }
       ...
       Convolve(imgHeight,imgWidth,&rbsonMask,imgBuffer,imgBuffer2);
