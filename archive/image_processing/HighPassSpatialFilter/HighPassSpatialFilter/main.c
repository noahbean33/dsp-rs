...
int main()
{
    ...
    const char newImgName[]= "girlface_hp.bmp";

    struct Mask highpassMask;
    signed char *tmp;
    int i;

    highpassMask.Cols=highpassMask.Rows =3;
    highpassMask.Data = (unsigned char *)malloc(9);

    /*  -1 -1 -1
        -1  9  -1
        -1  -1 -1*/
     int highpass_msk[] ={-1,-1,-1,-1,9,-1,-1,-1,-1};

     tmp = (signed char *)highpassMask.Data;
       for(int i =0;i<9;i++)
  {
      *tmp = highpass_msk[i];
      ++tmp;
       ...
       Convolve(imgHeight,imgWidth,&highpassMask,imgBuffer,imgBuffer2);
