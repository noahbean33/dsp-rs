...
CSLTestWindow::CSLTestWindow()
{
  ...
  HLOCAL HDSignalLocalMem, HDFreqRLocalMem, HDFreqILocalMem, HDCoeffLocalMem,
      HDResultLocalMem;    // Double type data array
...
afx_msg void CSLTestWindow::OnGenerateSine()
{
  ...
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, (SLData_t)0.9, SIGLIB_FILL, (SLData_t)0.01562, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO,
                     &SignalPhase, SIGLIB_NULL_DATA_PTR, Signal.GetSize());
...
afx_msg void CSLTestWindow::OnGenerateTriangle()
{
  ...
  HBuffer = Signal.GetHandle();
  BufferPtr = (SLData_t*)::LocalLock(HBuffer);
  SDA_SignalGenerate(BufferPtr, SIGLIB_TRIANGLE_WAVE, (SLData_t)0.9, SIGLIB_FILL, (SLData_t)0.01562, SIGLIB_ZERO, SIGLIB_ZERO, SIGLIB_ZERO,
                     &SignalPhase, SIGLIB_NULL_DATA_PTR, Signal.GetSize());
  ::LocalUnlock(HBuffer);    // Unlock the sample array
...
afx_msg void CSLTestWindow::OnGenerateSquare()
{
  ...
  SDA_SignalGenerate(BufferPtr, SIGLIB_SQUARE_WAVE, (SLData_t)0.9, SIGLIB_FILL, (SLData_t)0.01562, SIGLIB_ZERO, SIGLIB_HALF, SIGLIB_ZERO,
                     &SignalPhase, SIGLIB_NULL_DATA_PTR, Signal.GetSize());
  ::LocalUnlock(HBuffer);    // Unlock the sample array
