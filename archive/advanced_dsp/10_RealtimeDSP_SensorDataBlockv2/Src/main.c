...
uint32_t num_of_blocks = INPUT_SIG_LEN /BLOCK_SIZE;
...
void processing_tsk(void *pvParameters)
{
	...
	while(1)
	{
		...
		/*Execute fir function for every block*/
		for(int i = 0; i < num_of_blocks; i++)
		{
			arm_fir_f32(&fir_desc,
						f32_input_pt + (i  * blocksize),
						f32_output_pt + (i * blocksize),
						blocksize
					);
		}
