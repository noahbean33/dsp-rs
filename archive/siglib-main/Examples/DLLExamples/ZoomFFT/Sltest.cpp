...
CSLTestWindow::CSLTestWindow()
{
  ...
  ClassName = AfxRegisterWndClass(CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW, SLTestApp.LoadStandardCursor(IDC_ARROW),
...
BOOL CSLTestApp::OnIdle(LONG lCount)
{
...
afx_msg void CSLTestWindow::OnPaint()
{
  ...
  CPaintDC PaintDC(this);    // 'this' is a keyword, that supplies
...
afx_msg int CSLTestWindow::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
afx_msg void CSLTestWindow::OnSize(UINT nType, int cx, int cy)
{
  UINT dummy;
  RECT Rect;
...
{
  CModalDialog Contents("ContentsBox", this);
  Contents.DoModal();
}
...
afx_msg void CSLTestWindow::OnGenerateSine()
{
  ...
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.2, SIGLIB_FILL, CARRIER_FREQUENCY - 0.015, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.3, SIGLIB_ADD, CARRIER_FREQUENCY - 0.01, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.4, SIGLIB_ADD, CARRIER_FREQUENCY - 0.005, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 1.0, SIGLIB_ADD, CARRIER_FREQUENCY, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0, Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.6, SIGLIB_ADD, CARRIER_FREQUENCY + 0.005, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.7, SIGLIB_ADD, CARRIER_FREQUENCY + 0.01, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
  SDA_SignalGenerate(BufferPtr, SIGLIB_SINE_WAVE, 0.8, SIGLIB_ADD, CARRIER_FREQUENCY + 0.015, 0.0, 0.0, 0.0, &SignalPhase, (SLData_t*)0,
                     Signal.GetSize());
...
afx_msg void CSLTestWindow::OnZoomFFTSignal()
{
  ...
  if (FirstTime) {
    SIF_ZoomFftSimple(CombFilter1Ptr, &CombFilter1Sum, CombFilter2Ptr, &CombFilter2Sum, &CombFilterPhase, SineTablePtr, &SineTablePhase,
                      FFTCoeffPtr, SIGLIB_NULL_ARRAY_INDEX_PTR, DECIMATION_FILTER_LENGTH, SINE_BUF_SIZE, FFT_LENGTH);
    FirstTime = 0;
  }
