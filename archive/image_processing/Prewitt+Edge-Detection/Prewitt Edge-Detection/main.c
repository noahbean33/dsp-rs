...
int main()
{
    ...
    const char newImgName[]= "lena_prwt_hor.bmp";

    struct Mask prwtMask;
    signed char *tmp;
    int i;

    prwtMask.Cols=prwtMask.Rows =3;
    prwtMask.Data = (unsigned char *)malloc(9);

    /* Vertical Edges
     -1  0  -1
     -1  0  -1
     -1  0  -1 */
     int prewitt_ver[] ={-1,0,1,-1,0,1,-1,0,1};
     ...
     /*Horizontal Edges
      -1 -1 -1
       ...
       int prewitt_hor[] ={-1,-1,-1,0,0,0,1,1,1};
       tmp = (signed char *)prwtMask.Data;
       for(i =0;i<9;++i){
        *tmp =prewitt_hor[i] ;
        ++tmp;
       }
       ...
       Convolve(imgHeight,imgWidth,&prwtMask,imgBuffer,imgBuffer2);
