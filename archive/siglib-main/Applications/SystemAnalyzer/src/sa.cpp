...
// JEScale //v28      : wxFrame ((wxFrame *) NULL, -1, Title, Position, Size,
// wxDEFAULT_FRAME_STYLE & ~ (wxRESIZE_BORDER | wxRESIZE_BOX | wxMAXIMIZE_BOX))
// JEScale        : wxFrame ((wxFrame *) NULL, -1, Title, Position, Size,
...
//       wxMINIMIZE_BOX | wxSYSTEM_MENU)
{
  ...
  if (screenX > 1080) {
    SetSize(wxRect(400, SA_TOP, (int)(screenX * 0.6),
                   (int)(screenY * 0.8)));    // X, Y, W, H - used for a resizeable border
  }

  else {
    SetSize(wxRect(SA_LEFT, SA_TOP, SA_WIDTH,
                   SA_HEIGHT));    // X, Y, W, H - used for a resizeable border
  }
...
void mainFrame::DisplayStatusLineText2(wxString& string)
{
  ...
  //  wxMessageBox ("Got Here", "mainFrame::DisplayStatusLineText2", wxOK |
  //  wxICON_EXCLAMATION, NULL);
  SetStatusText(string,
                2);    // This prints the location and magnitude at the cursor
}
...
EVT_SIZE(DisplayTextDialog::OnSize) END_EVENT_TABLE()
